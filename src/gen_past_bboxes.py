        ...
        if "MOT" in d:
            ...
            for seq in seqs:
                ...
                if osp.exists(seq_info_file):
                    with open(osp.join(seq_root, seq, "seqinfo.ini"), "r") as file:
                        ...
                        seq_width = int(
                            seq_info[
                                seq_info.find("imWidth=")
                                + 8 : seq_info.find("\nimHeight")
                            ]
                        )
                        seq_height = int(
                            seq_info[
                                seq_info.find("imHeight=")
                                + 9 : seq_info.find("\nimExt")
                            ]
                        )
                ...
                gen_past_files(seq_label_root, past_label_root, past_length, img_size)
            ...
            for p_seq in parent_seqs:
                ...
                for seq in seqs:
                    ...
                    gen_past_files(
                        seq_label_root, past_label_root, past_length, img_size
                    )
            ...
            past_label_root = label_root.replace("labels_with_ids", past_label)

            gen_past_files(label_root, past_label_root, past_length, img_size)

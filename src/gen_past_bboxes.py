...
def gen_past_files(seq_label_root, past_label_root, past_length=30, img_size=None, extract_fid=int, filename_fmt='{:06d}.txt'):
...
def main(seq_root, label_root, past_length=30, seq_label="img1", past_label="past"):
    ...
    for seq in seqs:
        ...
        past_label_root = seq_label_root.replace('labels_with_ids', past_label)
        ...
        for filename in sorted(glob.glob(seq_label_root + "/*.txt")):
            ...
            fid = filename.split('/')[-1].replace('.txt', '')
            bbox = np.loadtxt(filename, dtype=np.float64)

            if not osp.exists(seq_info_file):
                # get image
                if osp.exists(filename.replace('.txt', '.png')):
                    image_file = osp.exists(filename.replace('.txt', '.png'))
                elif osp.exists(filename.replace('.txt', '.jpg')):
                    image_file = osp.exists(filename.replace('.txt', '.jpg'))
                else:
                    continue

...
def analyze(acc, hist):
    groups = acc.mot_events.reset_index()[["FrameId", "Type", "HId", "OId"]].groupby(
        ["FrameId", "Type"]
    )
    all = {}
    part = {}
    ...
    for (fid, t), grp in groups:
        ...
        all_idx[fid][t] = list(zip(hId, oId))
...
def visualize(all, part, keys=[], filename="out.jpg", legends=["all", "part"]):
    ...
    all_values = list(
        np.array(list(total_all.values())) - np.array(list(total_part.values()))
    )

...
from models.losses import FocalLoss, IOUloss, TripletLoss
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            if opt.forecast:
                ...
                pasts_probs = probs[:, :past_length]
                futures_probs = probs[:, -future_length:]
                ...
                if opt.pasts_weight > 0 and len(batch["pasts"]):
                    ...
                    pasts_mask = pasts_mask.type(torch.float32)

                    # pasts_mask = pasts_mask.type(torch.float)
                    ploss = F.l1_loss(
                        pasts_probs, pasts_mask, reduction="mean"
                    )  # .sum() / (futures_mask.sum() + 1e-10)
                    # pasts_loss += ploss
                ...
                if opt.futures_weight > 0 and len(batch["futures"]):
                    ...
                    ploss = F.l1_loss(
                        futures_probs.type(torch.float32),
                        futures_mask,
                        reduction="mean",
                    )  # .sum() / (futures_mask.sum() + 1e-10)

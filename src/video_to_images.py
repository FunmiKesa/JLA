...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1):
    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            save_path = os.path.join(frames_dir, video_filename, "{:06d}.jpg".format(frame))  # create the save path
...
def vid(folder):
    ...
    for filename in dirs:
        ...
        image_folder = folder.replace('clips', 'images')
        ...
        #     if not ret:
        #         break
        #     image_name = filename.replace(".mp4", f"_{i:06d}.png")
        #     image_path = os.path.join(image_folder, image_name)
        #     print(os.getcwd())
        #     print(image_path)
        #     cv2.imwrite(image_path, frame)
        #     i += 1
        video_to_frames(video_path=filepath, frames_dir=image_folder, overwrite=False, every=5, chunk_size=100)
